	...
	return {
		...
		async adapt(builder) {
			if (
				...
				fs.existsSync(`${builder.config.kit.files.assets}/_routes.json`)
			) {
				throw new Error(
			...
			if (fs.existsSync(`${builder.config.kit.files.assets}/_headers`)) {
				throw new Error(
					`The _headers file should be placed in the project root rather than the ${builder.config.kit.files.assets} directory`
				);
			}

			if (fs.existsSync(`${builder.config.kit.files.assets}/_redirects`)) {
				throw new Error(
